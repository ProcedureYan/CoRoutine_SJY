...
{
    // 如果钩子没有启用，则直接调用原始的 I/O 函数并返回结果
    if(!sylar::t_hook_enable) 
    {
        return fun(fd, std::forward<Args>(args)...);
    }

    // 通过文件描述符 fd 获取对应的 FdCtx（文件描述符上下文）对象
    ...
    // 如果 FdCtx 不是套接字类型，或者用户为该套接字设置了非阻塞模式，则直接调用原始的 I/O 函数并返回
    if(!ctx->isSocket() || ctx->getUserNonblock()) 
    {
        return fun(fd, std::forward<Args>(args)...);
    }
    // 根据 timeout_so（超时选项）从上下文中获取设置的超时时间
    uint64_t timeout = ctx->getTimeout(timeout_so);

    // 创建一个 timer_info 类型的共享指针 tinfo，用于存储定时器状态（例如是否取消）
...
retry:  
    ...
    // 如果 I/O 操作返回 -1 且 errno == EAGAIN（资源暂时不可用），则表示操作尚未完成，可能需要等待
    if(n == -1 && errno == EAGAIN) 
    {
